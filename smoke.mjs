...
// падающий от смены формулировки, только мешает. Зацепки — data-атрибуты:
// имена классов теперь генерирует CSS Modules, и в разметке они хешированы.
// Пока протокол не прочитан, страницы нет вовсе — её закрывает заставка.
// В браузероподобной среде без сети приложение обычно застаёт именно её,
// поэтому проверяем не «что-то одно обязано быть», а что состояние законное:
// либо заставка, либо уже отрисованная страница с шапкой.
...
if (loading) {
    ...
    // Переключатель сетей: обе вкладки обязаны быть на месте, а неразвёрнутая
    // сеть — помечена, чтобы ожидания задавались до клика, а не после.
    // Solana развёрнута на девнете, поэтому пометки soon у неё больше нет —
    // проверяем только наличие обеих вкладок.
    check(
        'переключатель сетей отрисован',
        html.includes('TON') && html.includes('Solana'),
    );
    // Пулов на TON больше одного, и выбор между ними — единственный способ
    // добраться до второго. Вкладка, не попавшая в разметку, делает пул
    // недостижимым при полностью рабочих контрактах.
    check(
        'переключатель пулов отрисован',
        html.includes('aria-label="Pool"') || html.includes('tsTON'),
    );
    // Кнопка кошелька Solana обязана быть на своей вкладке — без неё
    // подключиться невозможно, а понять это можно только кликом.
    check(
